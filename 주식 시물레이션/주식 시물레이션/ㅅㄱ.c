...
//수수료 (1개)[s],뉴스 목록(뉴스 개수 *2){1}[n], 기본키(1)[j], 매도 매수(1){m} [이름 매도 매입 수량], 도움 (1)[i] 상승값[ss],하락 [ss1];
#include<stdio.h>
#include<time.h>
#include<Windows.h>
int main(void) {
	...
	char name, gagag, riqos;//주식 매도 매입 도움용 변수//이름 ,개수 ,매도 매입
	int b2[28] = {0,};
	int day = 0;
	...
	char a[28][20] = { {"모아 제약"},{"로아 전력"},{"중추 공학"},{"소마고교"},{"아리란화"},{"손화 제약"},{"일클루전"},{"남화 건설"},{"사살라루"},{"일락트로"},{"남화 마트"},{"일루 건설"},{"화호라사"},{"수면 협회"},{"삼상 협회"},{"타이거즈"},{"유토피아"},{"아티아리"},{"시호이아"},{"테레콘트"},{"블마넷잡"},{"언이라화"},{"드자리블"},{"화환 마트"},{"지니리아"},{"로디아블"},{"오브 리그"},{"개인 협회"} };//주식 각각 이름
...
주식판:
	...
	scanf_s("%s %s %s",&riqos,&name,&gagag);
	if (riqos == "매도") {
